...
import { Message, NotificationItem } from '../api';

export function useNotifications(currentConversationId?: string) {
  ...
  useEffect(() => {
    ...
    const handleNewMessage = (payload: { conversationId: string; message: Message }) => {
      ...
      const isOwnMessage = message.senderId === localStorage.getItem('user_id');
      const shouldNotify = !isCurrentConversation || !isWindowFocusedRef.current;

      if (!isOwnMessage && shouldNotify) {
        // Get sender name if available
        const senderName = (message as any).sender?.name || 'Ai đó';
  ...
  useEffect(() => {
    ...
    const handleSocialNotification = (notification: NotificationItem) => {
      const actor = (notification as any).actorName || 'Người dùng';
      let body = '';
