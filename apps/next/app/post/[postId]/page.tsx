...
export default function PostDetailPage({ params }: Props) {
  ...
  useEffect(() => {
    let mounted = true;
    let hasCached = false;
    // Try cached post first for faster paint / offline fallback
    try {
  ...
  if (!post && error) {
    return (
      <div style={{ minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", flexDirection: "column", gap: 12 }}>
        ...
        <button onClick={() => router.push("/feed")} style={{ padding: "8px 16px", borderRadius: 6, border: "1px solid #3a3b3c", background: "#0f172a", color: "#e5e7eb", cursor: "pointer" }}>
          Về bảng tin
        </button>
      </div>
    );
