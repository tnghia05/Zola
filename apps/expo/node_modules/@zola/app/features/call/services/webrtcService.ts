...
export class WebRTCService {
  ...
  private localIceQueue: any[] = [];
  public onRemoteStream: ((stream: any) => void) | null = null;
  private renegotiationScheduled: boolean = false;
  private icePhase: 0 | 1 | 2 = 0; // 0: STUN+TURN(UDP first), 1: TURN-UDP only, 2: TURN TCP/TLS only
  private lastLocalAnswer: any = null;
  private iceErrorCount: number = 0;
  private lastIceErrorTs: number = 0;
  private answering: boolean = false;
  private currentFacingMode: 'user' | 'environment' = 'user';
  private isSpeakerOn: boolean = false;
  private currentFilter: 'none' | 'beauty-smooth' | 'beauty-bright' | 'beauty-contrast' | 'fun-blur' | 'fun-vintage' | 'fun-sepia' | 'fun-bw' = 'none';
  private filterCanvas: HTMLCanvasElement | null = null;
  private filterContext: CanvasRenderingContext2D | null = null;
  private filterVideoElement: HTMLVideoElement | null = null;
...
// ════════════════════════════════════════════════════════════
  ...
  private getPreferredIceServers(): any[] {
    return [
  ...
  updateTargetUser(targetUserId: string) {
    if (!targetUserId || targetUserId === this.targetUserId) return;
...
// ════════════════════════════════════════════════════════════
  ...
  async initializeCall(): Promise<any> {
    if (Platform.OS === 'web') {
  ...
  private async initializeWebCall(): Promise<any> {
    try {
    ...
    // ═══════════════════════════════════════════════════════════
      ...
      try {
        ...
        const startPhase = startPhaseRaw != null ? Number(startPhaseRaw) as 0|1|2 : null;
      ...
      try {
        (window as any).pc = this.peerConnection;
        (window as any).localStream = this.localStream;
        (window as any).getReceivers = () => (this.peerConnection as any).getReceivers?.();
        (window as any).getSenders = () => (this.peerConnection as any).getSenders?.();
      } catch {}
      ...
      try {
        ...
        if (transceivers.length === 0 && (this.peerConnection as any).addTransceiver) {
          console.log('📞 [WEBRTC] No transceivers found, creating sendrecv transceivers (web)');
          const v = (this.peerConnection as any).addTransceiver('video', { direction: 'sendrecv' });
          const a = (this.peerConnection as any).addTransceiver('audio', { direction: 'sendrecv' });
          // Prefer codecs: VP9/H.264 for video (better compression), Opus for audio
          try {
            const prefer = (kind: 'audio'|'video', t: any) => {
              const codecs = (RTCRtpReceiver as any).getCapabilities?.(kind)?.codecs || [];
              let filtered;
              if (kind === 'video') {
                // Ưu tiên VP9 (tốt nhất), sau đó H.264, cuối cùng VP8
                const vp9 = codecs.filter((c:any)=> /vp9/i.test(c.mimeType));
                const h264 = codecs.filter((c:any)=> /h264|avc/i.test(c.mimeType));
                const vp8 = codecs.filter((c:any)=> /vp8/i.test(c.mimeType));
                filtered = vp9.length ? vp9 : (h264.length ? h264 : vp8);
              } else {
                filtered = codecs.filter((c:any)=> /opus/i.test(c.mimeType));
              }
          ...
          try {
            ...
            transceivers.forEach((t: any) => {
              ...
              if (kind) prefer(kind as 'audio'|'video', t);
            });
          } catch {}
        }
      } catch (e) {
      ...
      try {
        ...
        for (const s of senders) {
          if (s.track?.kind === 'video') {
            ...
            p.encodings = [{ 
              maxBitrate: 2_500_000, // 2.5 Mbps cho Full HD
              minBitrate: 500_000,    // Min 500 kbps
              maxFramerate: 30,      // 30fps
              scaleResolutionDownBy: 1, // Không scale down
              // Low latency mode
              networkPriority: 'high' as any
            }];
          ...
          if (s.track?.kind === 'audio') {
            const p = s.getParameters();
            p.encodings = [{ 
              maxBitrate: 64_000,    // 64 kbps cho audio chất lượng cao
      ...
      try {
        const senders = (this.peerConnection as any).getSenders?.() || [];
        console.log('📞 [WEBRTC] Current senders:', senders.map((s:any)=>s.track?.kind+':'+s.track?.readyState));
      } catch {}
      ...
      this.peerConnection.ontrack = (event: any) => {
        ...
        if (this.remoteStream) {
          ...
          console.log('📞 [WEBRTC] Remote stream now has tracks:', tracks.map((t:any)=>t.kind+':'+t.readyState));
      ...
      this.peerConnection.oniceconnectionstatechange = () => {
        ...
        const state = this.peerConnection.iceConnectionState as any;
        if (state === 'failed') {
          ...
          setTimeout(() => {
            const cur = (this.peerConnection as any).iceConnectionState;
            if (cur === 'disconnected') {
      ...
      (this.peerConnection as any).onicecandidateerror = (event: any) => {
        try {
  ...
  private async initializeMobileCall(): Promise<any> {
    try {
      ...
      const stream = await mediaDevices.getUserMedia({
        video: {
          facingMode: 'user',
          width:  { ideal: 1920, min: 1280 } as any,
          height: { ideal: 1080, min: 720 } as any,
      ...
      (this.peerConnection as any).ontrack = (event: any) => {
        ...
        if (this.remoteStream) {
          console.log('📞 [WEBRTC][MOBILE] Remote tracks:', this.remoteStream.getTracks().map((t:any)=>t.kind+':'+t.readyState));
          this.onRemoteStream?.(this.remoteStream);
        } else {
  ...
  async createOffer(options?: RTCOfferOptions): Promise<void> {
    try {
  ...
  async handleOffer(offer: any): Promise<void> {
    try {
      ...
      await this.peerConnection.setLocalDescription({ type: answer.type, sdp: munged } as any);
  ...
  async handleAnswer(answer: any): Promise<void> {
    try {
      ...
      if (this.peerConnection.signalingState === 'stable') {
        const rd: any = (this.peerConnection as any).remoteDescription;
        if (rd?.type === 'answer') {
      ...
      try {
        ...
        if (receivers.length > 0) {
          if (!this.remoteStream) this.remoteStream = new MediaStream();
          receivers.forEach((r: any) => r.track && !this.remoteStream.getTracks().includes(r.track) && this.remoteStream.addTrack(r.track));
          this.onRemoteStream?.(this.remoteStream);
        }
      } catch (e) {
  ...
  private mungeSdpForPhase(sdp: string): string {
    try {
      ...
      for (const line of lines) {
        if (line.startsWith('a=candidate:')) {
          ...
          if (isIpv6) continue; // drop ipv6
          if (this.icePhase >= 1 && !(isRelay)) continue; // keep relay only in strict phases
          if (this.icePhase >= 1 && (isHost || isSrflx)) continue;
        }
        out.push(line);
      }
      return out.join('\r\n');
  ...
  private buildIceServers(phase: 0 | 1 | 2) {
    // Allow runtime TURN override via window.TURN_CONFIG or localStorage 'turn_config'
    let runtimeCfg: any = null;
    let forceHostV4: string | null = null;
    try {
      runtimeCfg = (typeof window !== 'undefined' && (window as any).TURN_CONFIG) || null;
      if (!runtimeCfg && typeof window !== 'undefined') {
        const raw = localStorage.getItem('turn_config');
        if (raw) runtimeCfg = JSON.parse(raw);
        forceHostV4 = localStorage.getItem('turn_host_v4');
      }
    } catch {}

    const buildFrom = (host: string, username: string, credential: string) => {
  ...
  private async fetchMeteredIceServers(): Promise<any[]> {
    try {
      ...
      if (Array.isArray(servers)) {
        let forceHostV4: string | null = null;
        try { forceHostV4 = (typeof window !== 'undefined') ? localStorage.getItem('turn_host_v4') : null; } catch {}
        servers.forEach((s: any) => {
          const toArray = (u: any) => Array.isArray(u) ? u : (u ? [u] : []);
          const urls: string[] = toArray(s.urls);
          const filtered = urls.filter((u: string) => u.startsWith('turns:'));
          if (filtered.length > 0) s.urls = filtered;
        });
        // Normalize: ensure every entry has urls[] and username/credential
        const fallbackUser = servers.find((s: any) => s?.username)?.username || 'ac1c979785feb61d89d10f9e';
        const fallbackCred = servers.find((s: any) => s?.credential)?.credential || 'lfBd3uXzDApKCXU/';
        const normalized: any[] = [];
        servers.forEach((s: any) => {
          const toArray = (u: any) => Array.isArray(u) ? u : (u ? [u] : []);
          let urls: string[] = toArray(s.urls).filter((u: string) => !!u);
          urls = urls.filter((u: string) => u.startsWith('turns:'));
          if (urls.length === 0) return; // skip non‑TURN rows
        ...
        normalized.forEach((e: any) => (servers as any).push(e));
        ...
        if (forceHostV4 && /^(\d+\.){3}\d+$/.test(forceHostV4)) {
          ...
          if (first) {
            const toArray = (u: any) => Array.isArray(u) ? u : (u ? [u] : []);
            const urls: string[] = toArray(first.urls) || [];
        ...
        try {
          ...
          if (disableTls) {
            servers.forEach((s: any) => {
              const toArray = (u: any) => Array.isArray(u) ? u : (u ? [u] : []);
              const urls: string[] = toArray(s.urls);
              s.urls = urls.filter((u: string) => !u.startsWith('turns:') && !u.includes('transport=tcp'));
            });
          }
        } catch {}
        const hasTcp = servers.some((s: any) =>
          Array.isArray(s.urls)
            ? s.urls.some((u: string) => u.startsWith('turns:'))
            : typeof s.urls === 'string' && (s.urls as string).startsWith('turns:')
        );
        if (!hasTcp) {
          // try to infer host from first entry
          const first = servers[0];
          const urls = Array.isArray(first?.urls) ? first.urls : [first?.urls].filter(Boolean);
          const hostMatch = urls?.map((u: string) => /turns?:([^:]+):/.exec(u) || /stun:([^:]+):/.exec(u))
  ...
  private emitIceCandidate(candidate: any) {
  ...
  private async restartIceWithPhase(phase: 0 | 1 | 2) {
    try {
      this.icePhase = phase;
      ...
      if (typeof (this.peerConnection as any).setConfiguration === 'function') {
  ...
  private handleIceError(event: any) {
    const now = Date.now();
  ...
  private attachExistingRemoteTracks(): void {
    try {
      if (!this.peerConnection || typeof (this.peerConnection as any).getReceivers !== 'function') {
        return;
      }
      const receivers = (this.peerConnection as any).getReceivers() || [];
      const tracks = receivers.map((r: any) => r.track).filter((t: any) => !!t);
      console.log('📞 [WEBRTC] attachExistingRemoteTracks receivers:', receivers.length, 'tracks:', tracks.map((t:any)=>t?.kind+':'+t?.readyState));
  ...
  async switchCamera(): Promise<boolean> {
    try {
      ...
      if (this.peerConnection) {
        const sender = this.peerConnection.getSenders().find((s: any) => 
  ...
  async applyFilter(filterType: 'none' | 'beauty-smooth' | 'beauty-bright' | 'beauty-contrast' | 'fun-blur' | 'fun-vintage' | 'fun-sepia' | 'fun-bw'): Promise<void> {
    ...
    try {
      ...
      const processFrame = () => {
        if (!this.filterVideoElement || !this.filterCanvas || !this.filterContext) return;
        
        if (this.filterVideoElement.readyState >= 2) {
          // Draw video frame to canvas
          this.filterContext.drawImage(
            this.filterVideoElement,
            0, 0,
          ...
          const imageData = this.filterContext.getImageData(
            0, 0,
            this.filterCanvas.width,
            this.filterCanvas.height
          );
  ...
  endCall(): void {
    ...
    if (this.filterStream) {
      this.filterStream.getTracks().forEach((track: any) => track.stop());
      this.filterStream = null;
    }
