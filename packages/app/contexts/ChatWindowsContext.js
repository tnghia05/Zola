...
import { getOpponentInfo, getCurrentUserId, getUserById, getMessages } from "../api";
const ChatWindowsContext = createContext(undefined);
export const ChatWindowsProvider = ({ children }) => {
    const [chatWindows, setChatWindows] = useState([]);
    const [minimizedChats, setMinimizedChats] = useState(new Set());
    const openChat = (userId, userName, userAvatar, isOnline) => {
        setChatWindows((prev) => {
    ...
    return (_jsx(ChatWindowsContext.Provider, { value: {
            chatWindows,
            openChat,
            closeChat,
            minimizeChat,
