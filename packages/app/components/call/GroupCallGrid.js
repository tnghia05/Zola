...
function ParticipantVideoTile({ participant, remoteVideoTrack }) {
    ...
    useEffect(() => {
        ...
        if (!participant.isLocal && remoteVideoTrack)
            return;
    ...
    return (_jsxs("div", { className: "participant-video-tile", children: [_jsx("video", { ref: videoRef, autoPlay: true, playsInline: true, muted: participant.isLocal, className: "participant-video", 
                // Performance optimizations
                style: {
                    transform: 'translateZ(0)', // Force GPU acceleration
                    willChange: 'transform', // Hint for browser optimization
                } }), _jsxs("div", { className: "participant-info", children: [participant.avatar ? (_jsx("img", { src: participant.avatar, alt: participant.name || participant.participantId, className: "participant-avatar" })) : (_jsx("div", { className: "participant-avatar-placeholder", children: (participant.name || participant.participantId).charAt(0).toUpperCase() })), _jsx("span", { className: "participant-name", children: participant.isLocal ? 'Bạn' : (participant.name || participant.participantId) })] })] }));
}
export function GroupCallGrid({ participants, localStream, localParticipantId, remoteTracks }) {
    ...
    return (_jsx("div", { className: `group-call-grid ${gridClass}`, children: allParticipants.map((participant) => {
            const tracks = remoteTracks?.get(participant.participantId);
            return (_jsx(ParticipantVideoTile, { participant: participant, remoteVideoTrack: participant.isLocal ? undefined : tracks?.videoTrack }, participant.participantId));
        }) }));
}
