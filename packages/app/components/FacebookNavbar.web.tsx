...
export const FacebookNavbarWeb = ({ currentUser, onLogout }: FacebookNavbarWebProps) => {
  ...
  const isActive = (path: string) =>
    pathname === path || (path === "/feed" && pathname === "/");
  ...
  return (
    <header className="fb-navbar">
      ...
      <div className="fb-navbar-center">
        <button
          className={`fb-nav-tab ${isActive("/feed") ? "fb-nav-tab--active" : ""}`}
          onClick={() => navigate("/feed")}
          title="Home"
        >
        ...
        <button
          className={`fb-nav-tab ${isActive("/conversations") ? "fb-nav-tab--active" : ""}`}
      ...
      <div className="fb-navbar-right">
        ...
        >
          <svg width="20" height="20" viewBox="0 0 20 20" fill="currentColor">
            <path d="M8 2a1 1 0 011 1v3h2V3a3 3 0 00-3-3H5a3 3 0 00-3 3v14a3 3 0 003 3h3a3 3 0 003-3v-3H9v3a1 1 0 01-1 1H5a1 1 0 01-1-1V3a1 1 0 011-1h3zm5.293 5.293L16.586 10l-3.293 2.707 1.414 1.414L20.414 10l-5.707-5.121-1.414 1.414z"></path>
          </svg>
        </button>
      </div>
    </header>
