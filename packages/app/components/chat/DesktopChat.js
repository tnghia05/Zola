...
export function DesktopChat({ conversationId, conversationName, isGroup, groupAvatar, groupMemberCount, memberIds, onTypingStart, onTypingStop, onToggleInfo, isInfoVisible, }) {
    // NOTE: navigate logic từ react-router-dom đã bỏ, Next sẽ điều hướng ở cấp trang.
    const { messages, loading, error, currentUserId, hasMore, sendMessage, markConversationAsRead, loadOlder, reactToMessage, togglePinMessage, editMessageContent, deleteMessageById, revokeMessageById, starMessageById, } = useChatMessages(conversationId);
    ...
    return (_jsxs(_Fragment, { children: [_jsx(ChatLayout, { header: _jsx(ChatHeader, { title: headerTitle, subtitle: headerSubtitle, avatar: isGroup ? groupAvatar : opponentInfo?.avatar, statusDotColor: !isGroup && opponentInfo?.status === 'online' ? '#10B981' : undefined, onVideoCall: handleVideoCall, onAudioCall: handleAudioCall, onToggleInfo: onToggleInfo, isInfoVisible: isInfoVisible, activeCall: activeCall ? {
                        id: activeCall.id,
                        type: activeCall.type,
                        callType: activeCall.callType,
                        livekitRoomName: activeCall.livekitRoomName,
                    } : null, onJoinCall: handleJoinCall }), messageArea: _jsxs(_Fragment, { children: [_jsx(MessageList, { messages: messages, currentUserId: currentUserId, loading: loading, hasMore: hasMore, opponentName: opponentInfo?.name, opponentAvatar: opponentInfo?.avatar, onLoadMore: loadOlder, onReply: (message) => {
                                setEditingMessage(null);
                                setReplyTarget(message);
                                setComposerText('');
                            }, onReact: (messageId, emoji) => reactToMessage(messageId, emoji), onTogglePin: (messageId, shouldPin) => togglePinMessage(messageId, shouldPin), onEdit: (message) => {
                                setReplyTarget(null);
                                ...
                                loadOlder();
                            }, focusMessageId: pendingScrollId, onFocusHandled: () => setPendingScrollId(null) }), _jsx(TypingIndicator, { users: typing.typingUsers })] }), composer: _jsx(MessageComposer, { disabled: loading, replyPreview: replyTarget?.text, editingLabel: editingMessage ? 'Chỉnh sửa tin nhắn' : undefined, value: composerText, onValueChange: (value) => {
                    ...
                    }, onCancelReply: () => setReplyTarget(null), onCancelEdit: () => {
                        setEditingMessage(null);
                        setComposerText('');
                    }, onSend: handleSend }), rightPanel: !isGroup && isInfoVisible
                    ? (_jsx(DirectChatInfoPanel, { conversation: {
                            _id: conversationId,
                            isGroup: false,
                            title: headerTitle,
                        }, opponentName: opponentInfo?.name, opponentAvatar: opponentInfo?.avatar, messages: messages }))
                    : null }), _jsx(ForwardMessageDialog, { message: forwardingMessage, onClose: () => setForwardingMessage(null), onForwarded: () => {
