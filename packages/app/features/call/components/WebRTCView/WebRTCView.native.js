...
export const WebRTCView = ({ streamURL, style, mirror, onVideoStarted, muted = true }) => {
    const videoRef = useRef(null);
    const lastStreamId = useRef(null);
    if (Platform.OS === 'web') {
        ...
        useEffect(() => {
            ...
            if (video && streamURL) {
                ...
                console.log('📞 [WEBRTCVIEW] Tracks:', streamURL.getTracks().map((t) => t.kind + ':' + t.readyState));
                lastStreamId.current = currentStreamId;
                // Only reset srcObject if changed to avoid DOM removal/remount issues
                const needsAttach = video.srcObject !== streamURL;
                if (needsAttach) {
                    try {
                        ...
                        video.src = URL.createObjectURL(streamURL);
                    }
                }
                video.autoplay = true;
                video.muted = !!muted;
                ...
                if (vTrack) {
                    ...
                    if (vTrack.onunmute === undefined) {
                        vTrack.addEventListener?.('unmute', onUnmute, { once: true });
                    }
                    else {
                        vTrack.onunmute = onUnmute;
                    }
                }
        ...
        return (_jsx("video", { ref: videoRef, style: {
                ...
                pointerEvents: 'none',
                ...style
            }, autoPlay: true, muted: true, playsInline: true }, (streamURL && (streamURL.id || streamURL.toString())) || 'no-stream'));
    }
    else {
        // Mobile implementation using RTCView
        if (RTCView) {
            return (_jsx(RTCView, { streamURL: streamURL, style: style, mirror: mirror }));
        }
        else {
