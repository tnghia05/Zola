...
import { getActiveCallForConversation, ActiveCallResponse } from '../api';
...
export function useActiveCall(conversationId: string | undefined): UseActiveCallResult {
  ...
  useEffect(() => {
    ...
    const handleCallIncoming = (data: { conversationId: string }) => {
      if (data.conversationId === conversationId) {
