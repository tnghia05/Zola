...
import { getMessages, createMessage, createMessageWithPayload, markAsRead, addReaction, removeReaction, pinMessage, unpinMessage, editMessage, deleteMessage, revokeMessage, starMessage, } from '../api';
import { getSocket } from '../socket';
import { registerChatSocket } from '../services/chatSocket';
const DEFAULT_PAGE_SIZE = 40;
const toChatMessage = (message) => ({
    ...message,
    // Chuẩn hóa để UI dễ render ảnh: ưu tiên imageUrl, fallback sang file.url
    imageUrl: message.imageUrl ?? message.file?.url,
});
const sortMessages = (msgs) => [...msgs].sort((a, b) => new Date(a.createdAt).getTime() - new Date(b.createdAt).getTime());
export function useChatMessages(conversationId) {
    ...
    const applyMessageUpdate = useCallback((messageId, updater) => {
        setAllMessages((prev) => {
            const next = prev
    ...
    const sendMessage = useCallback(async (payload) => {
        ...
        try {
            ...
            setAllMessages((prev) => {
                ...
                return sortMessages([...prev, toChatMessage(response)]);
            });
        }
        catch (err) {
    ...
    const loadOlder = useCallback(async () => {
        if (!hasMore || loadingOlder || !conversationId)
            return;
    ...
    const togglePinMessage = useCallback(async (messageId, shouldPin) => {
    ...
    const editMessageContent = useCallback(async (messageId, text) => {
    ...
    const revokeMessageById = useCallback(async (messageId) => {
        applyMessageUpdate(messageId, (msg) => ({
            ...msg,
            isRevoked: true,
    ...
    const starMessageById = useCallback(async (messageId) => {
