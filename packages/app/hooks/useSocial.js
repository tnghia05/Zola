...
import { checkPostLikedApi, createCommentApi, createPostApi, createStoryApi, getFeed, getNotificationsApi, getPostComments, getStoriesApi, getFriendSuggestionsApi, searchSocialApi, markStoryViewedApi, markNotificationsReadApi, deleteStoryApi, savePostApi, reportPostApi, getSavedPostsApi, unsavePostApi, reactToPostApi, removeReactionApi, } from "../api";
export const useFeed = () => {
    ...
    const loadMore = useCallback(async (reset = false) => {
        ...
        setError(null);
        try {
            const res = await getFeed(reset ? undefined : cursor || undefined);
    ...
    useEffect(() => {
        ...
        const handleReactionUpdate = (data) => {
            setItems((prev) => prev.map((p) => p._id === data.postId
                ? {
                    ...p,
            ...
            if (data.userId &&
                ...
                data.userId === window.__currentUserId) {
    ...
    const createPost = useCallback(async (content, media = [], visibility = "PUBLIC", taggedUsers) => {
    ...
    const updateReactionCounts = useCallback((postId, nextReaction, prevReaction) => {
        setItems((prev) => prev.map((post) => {
            if (post._id !== postId)
                return post;
    ...
    const selectReaction = useCallback(async (postId, reaction) => {
    ...
    const clearReaction = useCallback(async (postId) => {
        const prev = reactionMap[postId] || null;
        if (!prev)
            return;
    ...
    const reportPost = useCallback(async (postId, payload) => {
...
export const useStories = () => {
    ...
    useEffect(() => {
        ...
        const handleStoryDeleted = (payload) => {
            setStories((prev) => prev
                .map((group) => group.author._id === payload.authorId
                ? {
                    ...group,
        ...
        const handleStoryViewed = (payload) => {
            setStories((prev) => prev.map((group) => ({
                ...group,
                stories: group.stories.map((story) => story._id === payload.storyId
    ...
    const markStorySeen = useCallback(async (storyId) => {
        setStories((prev) => prev.map((group) => ({
            ...group,
            stories: group.stories.map((story) => story._id === storyId
...
export const useSocialSearch = () => {
    ...
    const [isLoading, setIsLoading] = useState(false);
    const [error, setError] = useState(null);
    const search = useCallback(async (keyword, type = "all") => {
...
export const useComments = (postId) => {
    ...
    const loadMore = useCallback(async (reset = false) => {
        ...
        setIsLoading(true);
        try {
            const res = await getPostComments(postId, reset ? undefined : cursor || undefined);
            setHasNext(res.hasNext);
            setCursor(res.nextCursor);
            setItems((prev) => (reset ? res.items : [...prev, ...res.items]));
        }
...
export const useSavedPosts = () => {
    ...
    const unsave = useCallback(async (postId) => {
...
export const useSocialNotifications = () => {
    ...
    const load = useCallback(async (reset = false) => {
        const res = await getNotificationsApi(reset ? undefined : cursor || undefined);
        setHasNext(res.hasNext);
        setCursor(res.nextCursor);
        setItems((prev) => (reset ? res.items : [...prev, ...res.items]));
        const unread = (reset ? res.items : [...items, ...res.items]).filter((n) => !n.isRead).length;
